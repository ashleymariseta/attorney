    ...
    list.sort((a, b) {
      switch (_sort) {
        case _SortKey.matters:
          return ((b['matters_count'] as num?)?.toInt() ?? 0)
              .compareTo((a['matters_count'] as num?)?.toInt() ?? 0);
        case _SortKey.outstanding:
          return (double.tryParse('${b['outstanding_total'] ?? 0}') ?? 0)
              .compareTo(double.tryParse('${a['outstanding_total'] ?? 0}') ?? 0);
        ...
        case _SortKey.lastActivity:
          return parseDate(b['last_consultation_at'] as String?)
            ...
            child: Container(
              ...
              child: DropdownButton<_SortKey>(
                ...
                items: const [
                  ...
                  DropdownMenuItem(value: _SortKey.matters, child: Text('Sort · Most matters')),
                  DropdownMenuItem(
                      value: _SortKey.outstanding, child: Text('Sort · Outstanding')),
                  DropdownMenuItem(value: _SortKey.invoiced, child: Text('Sort · Invoiced')),
                  DropdownMenuItem(
                      value: _SortKey.lastActivity, child: Text('Sort · Last activity')),
                ],
...
String _money(dynamic value) {
  ...
  final formatter = NumberFormat.currency(symbol: '\$', decimalDigits: n == n.truncate() ? 0 : 2);
...
class _StatStrip extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final retainer = clients.where((c) => c['relationship'] == 'retainer').length;
    final outstanding = clients.fold<double>(
        0, (s, c) => s + (double.tryParse('${c['outstanding_total'] ?? 0}') ?? 0));
    final invoiced = clients.fold<double>(
        0, (s, c) => s + (double.tryParse('${c['invoiced_total'] ?? 0}') ?? 0));
    return GridView.count(
      ...
      children: [
        _StatTile(
            label: 'Clients', value: '${clients.length}', icon: LucideIcons.users),
        _StatTile(
            label: 'On retainer',
            sub: 'active',
            value: '$retainer',
...
class _StatTile extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final color = tone == _StatTone.amber ? const Color(0xFFB45309) : AppColors.ink;
    return Container(
  ...
  Widget build(BuildContext context) {
    ...
    final lastConsult = lastConsultRaw == null
        ...
        : (DateTime.tryParse(lastConsultRaw) != null
            ? DateFormat('d MMM yyyy').format(DateTime.parse(lastConsultRaw).toLocal())
            : '—');
                              ...
                              child: const Row(
                                ...
                                children: [
                                  Icon(LucideIcons.award,
                                      size: 9, color: AppColors.brandDark),
                                  SizedBox(width: 3),
                                  Text(
                                    'RETAINER',
                                    style: TextStyle(
              ...
              children: [
                ...
                Expanded(
                  child: _Metric(
                    label: 'Outstanding',
                    value: _money(outstanding),
                    tone: outstandingValue > 0 ? _StatTone.amber : _StatTone.normal,
                  ),
                ),
                Expanded(
                  child: _Metric(label: 'Last', value: lastConsult, small: true),
                ),
              ],
            ),
          ],
...
class _ClientDetailSheetState extends ConsumerState<_ClientDetailSheet> {
        ...
        return Container(
          ...
          child: FutureBuilder<Map<String, dynamic>>(
            ...
            builder: (context, snap) {
              ...
              final client = (data['client'] as Map?)?.cast<String, dynamic>() ?? const {};
              return _buildShell(
                scrollController,
                client: client,
  ...
  Widget _buildDetailBody(
    ...
    final matters = ((data['matters'] as List?) ?? const []).cast<Map<String, dynamic>>();
    final payments = ((data['payments'] as List?) ?? const []).cast<Map<String, dynamic>>();
    final consultations =
        ((data['consultations'] as List?) ?? const []).cast<Map<String, dynamic>>();
      ...
      children: [
        ...
        Container(
          ...
          child: switch (_tab) {
            ...
            _Tab.consultations => _ConsultationsTab(consultations: consultations),
          },
        ),
...
class _OverviewTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final nextConsult = consultations.firstWhere(
      (c) {
...
class _InvoicesTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.stretch,
      children: [
        for (final p in payments) _InvoiceRow(payment: p),
      ],
    );
  }
}
...
class _InvoiceRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final invoiceNum = 'INV-${(payment['id'] as num?)?.toInt().toString().padLeft(5, '0') ?? '00000'}';
