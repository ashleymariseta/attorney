...
class GoogleAuthView(APIView):
    """Sign in with Google. Body: ``{id_token}`` (a Google ID token from the
    web / mobile sign-in). Verifies it, finds-or-creates the user (a client by
    default on first sign-in), and returns our JWT pair — same shape as login."""
    ...
    def post(self, request):
        ...
        token = (request.data.get('id_token') or request.data.get('credential') or '').strip()
        if not token:
            raise ValidationError('Missing Google id_token.')

        client_ids = dj_settings.GOOGLE_OAUTH_CLIENT_IDS
        if not client_ids:
            return Response({'detail': 'Google sign-in is not configured.'},
                            status=status.HTTP_503_SERVICE_UNAVAILABLE)
        try:
            # Verify signature, issuer and expiry; we check the audience against
            # ALL of our client IDs (web/android/ios) ourselves below.
        ...
        if idinfo.get('iss') not in ('accounts.google.com', 'https://accounts.google.com'):
            raise ValidationError('Wrong token issuer.')
        if idinfo.get('aud') not in client_ids:
            raise ValidationError('This Google token was not issued for this app.')

        email = (idinfo.get('email') or '').strip().lower()
        if not email or not idinfo.get('email_verified', False):
            raise ValidationError('Your Google account has no verified email.')

        domain = dj_settings.GOOGLE_ALLOWED_EMAIL_DOMAIN
        if domain and (idinfo.get('hd', '').lower() != domain) and not email.endswith('@' + domain):
            raise ValidationError(f'Sign-in is restricted to @{domain} accounts.')
        ...
        if user is None:
            user = User.objects.create_user(
                email=email,
                first_name=(idinfo.get('given_name') or '')[:120],
                last_name=(idinfo.get('family_name') or '')[:120],
                role=UserRole.CLIENT_INDIVIDUAL,
                email_verified=True,
            )
            ...
            if not user.first_name and idinfo.get('given_name'):
                user.first_name = idinfo['given_name'][:120]
                updates.append('first_name')
            if not user.last_name and idinfo.get('family_name'):
                user.last_name = idinfo['family_name'][:120]
        ...
        refresh = RefreshToken.for_user(user)
        audit(actor=user, action='auth.google', obj=user, meta={'created': created}, request=request)
        return Response({'access': str(refresh.access_token), 'refresh': str(refresh)})
